...
<script>
    function submitApplication() {
        ...
        if (plaintiffFileInput.files.length > 0) {
            fd.append('plantiff_citizenship_case_member', plaintiffFileInput.files[0]);
            formJson.file_uploads.push({
                field: 'plantiff_citizenship_case_member',
                oldName: plaintiffFileInput.files[0].name,
                newName: generateUniqueFileName('plantiff_citizenship_case_member', plaintiffFileInput.files[0].name)
            });
        }

        const defendantFileInput = document.getElementById('defendant_citizenship');
        if (defendantFileInput.files.length > 0) {
            fd.append('defendant_citizenship_case_member', defendantFileInput.files[0]);
            formJson.file_uploads.push({
                field: 'defendant_citizenship_case_member',
                oldName: defendantFileInput.files[0].name,
                newName: generateUniqueFileName('defendant_citizenship_case_member', defendantFileInput.files[0].name)
            });
        }

        const generalFileInput = document.getElementById('file_upload');
        for (let i = 0; i < generalFileInput.files.length; i++) {
            fd.append('general_files_case_member[]', generalFileInput.files[i]);
            formJson.file_uploads.push({
                field: 'general_files_case_member[]',
                oldName: generalFileInput.files[i].name,
                newName: generateUniqueFileName('general_files_case_member', generalFileInput.files[i].name)
            });
        }
