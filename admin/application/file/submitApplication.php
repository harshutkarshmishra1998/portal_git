...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $plaintiffCitizenship = $files['plantiff_citizenship_application_admin'] ?? null;
        $defendantCitizenship = $files['defendant_citizenship_application_admin'] ?? null;
        ...
        if ($plaintiffCitizenship && $plaintiffCitizenship['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($plaintiffCitizenship['tmp_name'], $newFilename)) {
                $movedFiles['plantiff_citizenship_application_admin'] = $newFilename;
                error_log("Plaintiff citizenship file moved successfully: " . $newFilename);
            } else {
                $movedFiles['plantiff_citizenship_application_admin'] = 'Error moving file.';
        ...
        if ($defendantCitizenship && $defendantCitizenship['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($defendantCitizenship['tmp_name'], $newFilename)) {
                $movedFiles['defendant_citizenship_application_admin'] = $newFilename;
                error_log("Defendant citizenship file moved successfully: " . $newFilename);
            } else {
                $movedFiles['defendant_citizenship_application_admin'] = 'Error moving file.';
            ...
            foreach ($generalFiles['tmp_name'] as $key => $tmpName) {
                if ($generalFiles['error'][$key] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($tmpName, $newFilename)) {
                        $movedFiles['general_files_application_admin'][$key] = $newFilename;
                        error_log("General file moved successfully: " . $newFilename);
                    } else {
                        $movedFiles['general_files_application_admin'][$key] = 'Error moving file.';
        ...
        $stmtStatus = $pdo->prepare("INSERT INTO application_status (reference_id, status, comment, editor_name, editor_email, editor_mobile, file_upload, created_at) VALUES (:ref_id, 'Pending', 'File Uploaded by Admin', :editor_name, :editor_email, :editor_mobile, :file_upload, NOW())");
